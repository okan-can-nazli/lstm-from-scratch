...
class LSTMCell:
    def __init__(self, input_size, stm_size):
        ...
        ####################################
        self.Wy = np.random.randn(1, stm_size) * 0.01  # Projects 128 → 1
    ...
    def forward_sequence(self, x_sequence, stm_init, ltm_init):
        ...
        self.caches = [] # <--- NEW: Create a list to hold our snapshots
        ...
        for x in x_sequence:
            ...
            # <--- NEW: Take a snapshot of the gates forward() just saved
            snapshot = (self.combined_input, self.forget_gate, self.input_gate, 
                        self.new_potential_memory, self.output_gate, self.ltm_prev, self.ltm_next)
            self.caches.append(snapshot)
    ...
    def backward_sequence(self, dstm_outputs):
        ...
        for t in reversed(range(len(dstm_outputs))):
            ...
            # <--- NEW: Restore the snapshot for THIS exact timestep
            (self.combined_input, self.forget_gate, self.input_gate, 
             self.new_potential_memory, self.output_gate, self.ltm_prev, self.ltm_next) = self.caches[t]
    ...
    def update_weights(self, grads, learning_rate=0.01):
        ...
        ########################3
        self.Wy -= learning_rate * grads['dWy']
        self.by -= learning_rate * grads['dby']